    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg0)' }}>
      ...
      <div style={{ maxWidth: 1600, margin: '0 auto', padding: '16px 20px 40px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ background: 'var(--bg1)', border: '1px solid var(--border)', borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <button
              onClick={() => { const next = !liveMode; setLiveMode(next); if (next) doLiveRefresh() }}
              style={{
            ...
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <input
                ...
                style={{
                  ...
                  cursor: 'text', opacity: liveMode ? 0.6 : 1,
                }}
              />
            </div>
          ))}
        ...
        <div className="panel">
          <div className="panel-header">
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <button onClick={exportCSV} disabled={!records.length} className="btn-ghost"
                style={{ fontSize: 11, padding: '4px 12px', opacity: records.length ? 1 : 0.4 }}>⬇ CSV</button>
              <button onClick={exportXLSX} disabled={!records.length} className="btn-ghost"
                style={{ fontSize: 11, padding: '4px 12px', opacity: records.length ? 1 : 0.4 }}>⬇ Excel</button>
            </div>
          </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                  ...
                  <tbody>
                    {pageRows.map((rec, i) => {
                      ...
                      const cell = (v, hi) => (
                        <td style={{ padding: '6px 10px', fontFamily: 'monospace', color: hi ? 'var(--text-1)' : 'var(--text-2)', whiteSpace: 'nowrap' }}>{v ?? '--'}</td>
                      )
                      return (
